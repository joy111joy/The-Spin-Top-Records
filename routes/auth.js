...
router.post("/", async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      if (await bcrypt.compare(req.body.password, user.password)) {
        ...
        if (DEBUG) {
          console.log("\n");
          console.log(
            "Copy and paste the following curl command to test the API."
          );
          console.log(
            `curl -H "Authorization: Bearer ${token}" -X GET http://localhost:3000/api/full/p/[keyword]`
          );
          console.log("\n");
        }
        myEventEmitter.emit(
        ...
        if (DEBUG)
          console.log(
            "auth.post.getLoginByUsername().try password: " + user.password
          );
        ...
        res.redirect("/auth");
        req.session.status = null; // Clear the status after redirect
        return;
      }
    }
    if (DEBUG) console.log("auth.getLoginByUsername().catch:", error);
    console.log(error);
    myEventEmitter.emit(
      "event",
      "auth.post",
    ...
    if (DEBUG) console.log("auth.post.getLoginByUsername().finally");
  }
});
      ...
      if (result.code === "23505" || result.code === 11000) {
        ...
        if (result.code === "23505") {
          ...
          myEventEmitter.emit(
            ...
            "auth.post /new",
            "INFO",
            `MongoDB unique violation: 11000`
          );
        ...
        if (DEBUG)
          console.log(`${constraint} already exists, please try another.`);
