...
router.post("/", async (req, res) => {
  try {
    ...
    if (await bcrypt.compare(req.body.password, user.password)) {
      ...
      if (DEBUG) {
        console.log(
          `curl -H "Authorization: Bearer ${token}" -X GET http://localhost:3000/api/auth/${user._id}`
        );
        console.log(
          `curl -d "password=xxxxx" -H "Authorization: Bearer ${token}" -X POST http://localhost:3000/api/auth/${user._id}`
        );
        console.log(
          `curl -H "Authorization: Bearer ${token}" -X DELETE http://localhost:3000/api/auth/${user._id}`
        );
      }
      myEventEmitter.emit(
        "event",
